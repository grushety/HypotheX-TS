...
- [ ] Run `algorithm-auditor` agent with paper references: Eckhardt 2005 Eq. 6 and Table 1 parameter recommendations; Tallaksen 1995 on recession analysis. Verify recursive formula is exactly Eq. 6; verify `b(t) ≤ Q(t)` constraint matches paper §2
- [ ] Run `code-reviewer` agent — no blocking issues
- [ ] `git commit -m "SEG-016: Eckhardt baseflow recursive filter (hydrology)"`
